...
function eval_selections {
    ...
    track=$2
    size=$3
    selections "$coll.$track.queries.qk3${size}" "greedy" 1000 > /tmp/greedy
    selections "$coll.$track.queries.qk3${size}" "exact" 1000 > /tmp/exact
...
function selection_times {
    ...
    for k in "${ks[@]}"; do
        ...
        cat "$DATA/intersections/$coll.$track.queries.qk3${size}.jl" \
